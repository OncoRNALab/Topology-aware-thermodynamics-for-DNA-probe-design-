...
NN_PARAMS = {
    'AA': {'dG37': -1.00, 'dH': -7.6, 'dS': -21.3},
    'AT': {'dG37': -0.88, 'dH': -7.2, 'dS': -20.4},
    'TA': {'dG37': -0.58, 'dH': -7.2, 'dS': -21.3},
...
# (retained for backward compatibility; not used when USE_RC_MAP = True)
USE_RC_MAP = True
...
def get_nn_params(dinuc):
    ...
    By default (``USE_RC_MAP = True``) the thermodynamically correct
    reverse-complement mapping is used for the 6 non-canonical
    dinucleotides (TT, TG, AC, AG, TC, CC).
